...
function QuestionFeedback({ correct, explanation }) {
  return (
    ...
    >
      ...
      {!correct && (
        <small>
          Slow down here: this is exactly the kind of distinction Network+
          likes to test.
        </small>
      )}
    </div>
    ...
    <>
      ...
      {beginnerBridge && (
        <section className="beginner-bridge" aria-label={`Beginner bridge for objective ${activity.objective}`}>
          <div>
            ...
            <h2>Before the details, lock in the idea.</h2>
          </div>
          <div className="beginner-bridge__grid">
            <article>
              <strong>Vocabulary</strong>
              <p>{beginnerBridge.vocabulary.join(" · ")}</p>
            </article>
            <article>
              <strong>Why it matters</strong>
              <p>{beginnerBridge.why}</p>
            </article>
            <article>
              <strong>Workplace example</strong>
              <p>{beginnerBridge.example}</p>
            </article>
            <article>
              <strong>Mental model</strong>
              <p>{beginnerBridge.mentalModel}</p>
            </article>
          </div>
        </section>
      )}
      {editorial && (
        <section className="editorial-expansion" aria-label={`Practice guidance for objective ${activity.objective}`}>
          ...
          <div className="editorial-expansion__grid">
            <article>
              <strong>Workplace example</strong>
              <p>{editorial.workplaceExample}</p>
            </article>
            <article>
              <strong>Common beginner trap</strong>
              <p>{editorial.misconception}</p>
            </article>
            <article>
              <strong>How Network+ may test it</strong>
              <p>{editorial.examReasoning}</p>
            </article>
          </div>
          {editorial.practiceCluster && (
            <div className="practice-cluster">
              <strong>{editorial.practiceCluster.title}</strong>
              <ul>
                {editorial.practiceCluster.prompts.map((prompt) => (
      ...
      {editorial?.practiceCluster?.prompts?.length > 0 && (
        <section className="lesson-recap" aria-label={`Before you move on from objective ${activity.objective}`}>
          <p className="eyebrow">Before you move on</p>
...
export function SubnettingActivity({ onComplete, completed, nextTitle }) {
  return (
    <>
      <SubnettingPractice />
